...
/// 3 从SkinnedMeshRenderer里取出mesh数据（该mesh数据应该是包含bone index的） 对每个bone（就是transform）计算变换矩阵
/// 4 将生成的矩阵数据存入纹理
...
public class gpu_instancing : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (mtrl_work_)
            Graphics.DrawMeshInstanced(instance_mesh, 0, mtrl_work_, matrices, gobj_line_count_ * gobj_count_perline_);//materices�Ǳ任����
    }
